...
if (Get-Command docker -ErrorAction SilentlyContinue) {
    ...
    ## Start the StartupExampleSql container on port 5433 with local dev password
    docker run -e 'ACCEPT_EULA=Y' -e 'SA_PASSWORD=P@ssword123!' -p 5433:1433 --network vs --name 'StartupExampleSql' -d mcr.microsoft.com/mssql/server
}
